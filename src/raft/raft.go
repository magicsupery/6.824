	...
	if rf.CurrentTerm > args.Term{
		...
		if args.PrevLogIndex == 0{
			...
			if rf.getLastLogIndex() < args.PrevLogIndex{
				...
				if prevLogTerm != args.PrevLogTerm{
					...
					for index > 0{
						if rf.Logs[index - 1].Term == prevLogTerm{
							reply.XIndex = index
						}else{
							break
						}
		...
		if reply.Success{
			if args.LeaderCommit > rf.commitIndex{
				rf.commitLogs(min(args.LeaderCommit, uint64(len(rf.Logs))))
			}
		}
...
func (rf *Raft) changeToLeader() {
	...
	for index, _ := range rf.peers{
		if index == rf.me{
			rf.matchIndex[index] = uint64(len(rf.Logs))
...
func (rf *Raft) sendAllAppendEntries(isHeartbeat bool){
	...
	self := rf.me
	rf.matchIndex[self] = uint64(len(rf.Logs))
...
func(rf *Raft) sendSingleAppendEntries(peer *labrpc.ClientEnd, index int, isHeartbeat bool){
	...
	if lastLogIndex >= nextIndex{
		...
		copy(log, rf.Logs[nextIndex - 1: lastLogIndex])
	...
	if ok{
		...
		if reply.Success{
			...
			if reply.XLen != -1{
				...
				if reply.XTerm != 0{
					pivot := reply.XIndex
					if rf.Logs[pivot - 1].Term != reply.XTerm{
...
func (rf *Raft) who() string {
	return fmt.Sprintf("(%d, %d, Logs %v, commitIndex %v) ", rf.me, rf.CurrentTerm, rf.Logs, rf.commitIndex)
}
...
func (rf *Raft) getPrevLogTerm(pivotIndex uint64) uint64{
	if pivotIndex == 0{
		...
		return rf.Logs[pivotIndex - 1].Term
	}
}
